    ...
    class BlogController extends Controller
    {
        ...
        public function store(StoreBlogRequest $request): RedirectResponse
        {
            try {
                ...
                $blog->save();
                return redirect()->route('instructor.dashboard')->with('message', 'Blog đã được gửi đi và cần chờ phê duyệt')->with('icon', 'success');
            } catch (\Exception $e) {
                return redirect()->route('instructor.dashboard')->with('message', 'Tạo Blog thất bại')->with('icon', 'error');
            }
        }
        ...
        private function saveImageToSystem(UploadedFile $file): string
        {
            ...
            $file->move(public_path(env('TMP_FOLDER_PATH')), $fileName);
        ...
        public function storeImage(Request $request): JsonResponse
        {
            if ($request->hasFile('upload')) {
                ...
                $url = asset(env('TMP_FOLDER_PATH') . $fileName);
